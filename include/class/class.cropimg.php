...
/*
	调用方法
	$obj=new Img();
	$Img_small = $obj->Img_BigToSamll($rPic,100,135,$sPic,1); //$rPic为源图片，$sPic为要生成的图片

	如果要修改图片背景色的，请修改此句程序 $white = @imagecolorallocate($bg, 255, 255, 255);
*/
...
class Img
{
	...
	var $mode; //图片显示模式 0=>等比例,1=>居中,2=>裁切,3=>变形;
	...
	function Img_BigToSamll($src_img,$new_img_w,$new_img_h,$new_img_name,$mode)
	{
        ...
        $src_img_w = $imageInfo[0]; 
		$src_img_h = $imageInfo[1];
        $imgtype = $imageInfo[2];
		...
		if($imgtype == 1) {
			$fg = imagecreatefromgif($src_img);
        } elseif($imgtype == 2) {
            $fg = imagecreatefromjpeg($src_img);
        } elseif($imgtype == 3) {
			$fg = imagecreatefrompng($src_img);
        }
		...
		if($mode == 0){
			...
			$new_img_w = ($new_img_w > $src_img_w) ? $src_img_w : $new_img_w;
			$new_img_h = ($new_img_h > $src_img_h) ? $src_img_h : $new_img_h;
			$srcW = $src_img_w;
			$srcH = $src_img_h;
			
			if ($srcW * $new_img_w > $srcH * $new_img_h) {
				$new_img_h = round($srcH * $new_img_w / $srcW);
			} else {
				$new_img_w = round($srcW * $new_img_h / $srcH);
			}

			$bg = imagecreatetruecolor( $new_img_w,$new_img_h );
			imagecopyresampled($bg, $fg, 0, 0, 0, 0, $new_img_w, $new_img_h, $src_img_w, $src_img_h);
