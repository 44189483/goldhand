...
$(function() {
	var orientationText = '<div class="orientationText" style="position:fixed;top:0;padding-top:20px;background:#000;z-index:1000000;width:100%;color:#FFFFFF;font-size:2em;text-align:center;height:100%;lin-height:100%;vertical-align: middle;"><div class=""><img src="/templates/img/rotate.png" width="148"></div>请调整到合适屏幕阅读<div>'
	...
	$('.handimg').bind(touchEvents.touchend, function(event) {
		...
		if (n < 985) {
			...
			$(".status").html('less');
			//AJAX无刷新
			$.post("../" + app + "/ajax.php?type=getPid", function(data) {
				$(".seePrize").html(data);
			});
		}
		if (n >= 985 && n <=1014) {
	...
	//--------------------------登录提示start---------------------------------------
	doPlaceholder($('.name'), '请输入您的姓名');
	doPlaceholder($('.company'), '请输入您的公司名称');
	doPlaceholder($('.phone'), '请输入您的手机号');

	function doPlaceholder(elem, text) {

		elem.find('input').focus(function() {
			elem.css('border-bottom-color', '#f7caff');
			$(this).attr('placeholder', '');
		});
		elem.find('input').blur(function() {
			elem.css('border-bottom-color', '#9d69a6');
...
$(document).ready(function() {
	if ($(".btn_again,.bestTouch-btn").length > 0) {
		$(".btn_again,.bestTouch-btn").click(function() {
			$(".alertpage").show();
		})
	};
...
function checkform(form) {
	...
	if (errorText == '') {
		...
		var endpos = errorText.lastIndexOf('、');
		if (endpos == errorText.length - 1) {
			errorText = errorText.substring(0, endpos);
		}
		$('.loginPop-container-error').text(errorText);
		$('.loginPop').fadeIn();
		return false;
	}
...
function share() {
	...
	if (status == 'less') {
		...
		shareTitle = '完美的10秒，完美的10秒，完美的10秒，金手指果然名不虚传，快去领取你的奖励吧！';
	} else if (status == 'more') {
		shareTitle = '手一滑，竟然超过金手指' + lasttime + '的时间！胜败虽乃兵家常事，但强迫症的我必须重新来过！';
	} else {
		shareTitle = '金手指';
	}
